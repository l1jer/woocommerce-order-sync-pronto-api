...
#### 1.6.9
- **Optimization:** Removed unnecessary debug check and simplified scheduled event system
  - **Removed Debug Check:** Eliminated automatic debug logging that was causing log spam
  - **Simplified System:** Removed `debug_scheduled_events()` method that ran every 30 minutes
  - **Added Manual Health Check:** New `health_check_scheduled_events()` method for manual troubleshooting only
  - **Reduced Log Noise:** System now only logs when events actually run or when manually checked
  - **Better Performance:** Eliminated unnecessary system calls and logging overhead
  - **Cleaner Logs:** Removed hundreds of repetitive debug messages from log files
  - **Trust Server Cron:** System now relies on server cron without constant monitoring

#### 1.6.8
- **Fix:** Resolved excessive scheduling and logging issues in scheduled event system (Option 1 Implementation)
  - **Fixed Multiple Initialization:** Added static flag to prevent repeated system initialization
  - **Reduced Debug Frequency:** Fixed debug method to run every 30 minutes instead of every 2 minutes
  - **Prevented Excessive Rescheduling:** Added 1-hour cooldown between scheduling attempts
  - **Improved WordPress Cron Handling:** Changed warning to info message for disabled WordPress cron (expected with server cron)
  - **Added Server Cron Configuration Check:** New method to verify wp-cron.php accessibility and provide setup guidance
  - **Enhanced Logging Context:** Added timestamps and better context information to all log entries
  - **Optimized Performance:** Reduced unnecessary system calls and log spam
  - **Better Error Handling:** Improved error detection and reporting for scheduling issues

#### 1.6.7
- **Feature:** Debugged and enhanced scheduled event system for reliable automated shipment number retrieval (Task 1.6.7)
  - **Issue Identified:** Scheduled sync events were not triggering at expected times due to multiple system issues
  - **Fixed Critical Bug:** Missing variable declaration in `run_scheduled_sync()` method that prevented execution
  - **Enhanced Logging:** Migrated from `wc_get_logger()` to new `WCOSPA_Logger` system for better debugging
  - **Added Comprehensive Debugging:** New `debug_scheduled_events()` method checks scheduled events every 30 minutes
  - **Improved Scheduling Logic:** Enhanced `schedule_sync_events()` with detailed logging and verification
  - **Added Event Verification:** Automatic verification after scheduling to ensure events are properly registered
  - **WordPress Cron Detection:** Automatic detection and warning if WordPress cron is disabled
  - **Emergency Rescheduling:** Automatic rescheduling if no future events are found after sync completion
  - **Detailed Event Information:** New `get_scheduled_events_info()` method for comprehensive debugging
  - **Manual Testing:** Added `test_scheduled_sync()` method for manual testing of scheduled events
  - **Robust Error Handling:** Better error detection and logging for scheduling failures
  - **Timezone Accuracy:** Verified Sydney timezone handling throughout the scheduling system
  - **Performance Optimization:** Reduced unnecessary rescheduling to prevent conflicts

#### 1.6.6
- **Feature:** Refactored plugin logging system to use dedicated log files (Task 1.6.6)
  - **New Dedicated Logging:** Created `WCOSPA_Logger` class that writes all logs to plugin's own directory (`wp-content/plugins/woocommerce-order-sync-pronto-api/logs/`)
  - **Searchable Log Format:** All log entries include `[WCOSPA]` tag for easy identification and debugging
  - **Automatic Log Rotation:** Logs are automatically rotated when they exceed 10MB to prevent excessive file sizes
  - **7-Day Retention Policy:** Old log files are automatically deleted after 7 days to prevent storage bloat
  - **Enhanced Security:** Log directory protected with `.htaccess` and `index.php` files to prevent direct access
  - **Sydney Timezone:** All log timestamps use Sydney timezone for consistency with business operations
  - **Structured Context:** Logs include relevant context data (order IDs, error codes, etc.) for better debugging
  - **Fallback Mechanism:** Falls back to WordPress debug log if custom log files are not writable
  - **Admin Integration:** Provides methods for reading and managing log files from admin interface
  - **WordPress Best Practices:** Follows WordPress coding standards and security practices
  - **Backward Compatibility:** Existing logging calls continue to work while being migrated to new system
  - **Performance Optimized:** Efficient file operations with proper locking to prevent corruption

#### 1.6.5
- **Feature:** Implemented robust 524 timeout error detection and handling (Task 1.6.5)
  - **Problem Solved:** SiteGround server limitation causing 524 timeout errors for HTTP requests exceeding 120 seconds was being ignored, allowing processes to continue as if requests succeeded
  - **Solution Implemented:** Added comprehensive 524 timeout detection in all API methods (`sync_order`, `fetch_order_status`, `get_pronto_order_details`)
  - **New Method:** `check_for_524_timeout()` in `WCOSPA_API_Client` detects 524 errors and other server timeout errors (502, 503, 504)
  - **Enhanced Error Handling:** All API methods now properly detect and return specific WP_Error objects for timeout errors
  - **Improved Logging:** Comprehensive error logging with detailed context including method, order ID, response codes, and server timeout limits
  - **Processing Protection:** Shipment number fetching now stops immediately when 524 errors occur to prevent false success reporting
  - **Bulk Processing Safety:** Bulk operations halt immediately on critical server errors to prevent cascading failures
  - **Automatic Processing Safety:** Automatic shipment processing stops when server timeout errors are detected
  - **User Feedback:** Clear error messages distinguish between regular API errors and critical server timeout issues
  - **Documentation:** Technical implementation prevents further processing on 524 errors as specified in task requirements

#### 1.6.4
- **Feature:** Updated scheduled sync action with new timing requirements (Task 1.6.4)
  - **Monday to Thursday:** Runs at 9:00 AM, 12:00 PM, 2:00 PM, and 5:30 PM Sydney time
  - **Friday:** Runs at 9:00 AM and 12:00 PM Sydney time
  - **Weekend:** No scheduled sync actions (Saturday and Sunday are skipped)
  - Created new `WCOSPA_Scheduled_Sync_Handler` class to manage the complex scheduling logic
  - Replaced old daily morning sync (6:00 AM) and shipment tracking schedule (11:55 AM & 4:55 PM)
  - Each scheduled sync runs both order processing and shipment tracking functions
  - Automatic rescheduling ensures continuous operation week after week
  - Comprehensive logging for all scheduled events and execution
  - Proper cleanup of old scheduled events during plugin deactivation
  - Robust timezone handling ensuring accurate Sydney time scheduling

#### 1.6.3
- **Feature:** Added bulk "Obtain Shipping Number" button to the Orders admin page (Task 1.6.3)
  - New button appears next to the "Sync Processing Orders" button on the WooCommerce orders list page
  - Processes all orders currently in "Preparing to Ship" status that have Pronto order numbers but no shipment numbers
  - Uses existing `WCOSPA_Shipment_Handler::fetch_shipment_number()` functionality without creating new functions
  - Processes orders sequentially with 1-second delay between each order as required by task specifications
  - Only proceeds to next order after attempting to obtain shipment number for previous order
  - Provides comprehensive user feedback with detailed success/error notices
  - Includes confirmation dialog warning about sequential processing time
  - Comprehensive error handling and logging for each order processed
  - Follows same security and UI patterns as other bulk operations
